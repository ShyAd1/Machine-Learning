...
ARBOL = {
    ...
    "no": {  # No es vivo
        ...
        "no": {
            "q": "¿Se utiliza para comida o es comida? (sí/no): ",
            "si": {
                "q": "¿Es dada por la naturaleza? (sí/no): ",
                "si": None,
                "no": {
                    "q": "¿Lleva más de un ingrediente? (sí/no): ",
                    "si": None,
                    "no": None,
                },
            },
...
def recorre_arbol(
    ...
    """
    Recorre el árbol hasta 5 niveles o hasta llegar a una hoja.
    Devuelve lista de pares (pregunta, respuesta) en orden.
    """
    if historial is None:
        historial = []
    if nodo is None or nivel > 5 or contador.restantes() <= 0:
        return historial
